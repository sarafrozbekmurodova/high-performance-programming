...
int read_doubles_from_file(int n, double *p, const char *fileName) {
    ...
    if (!input_file) {
        printf(
            "read_doubles_from_file error: failed to open input file '%s'.\n",
            fileName);
        return -1;
    }
    ...
    if (fileSize != n * sizeof(double)) {
        printf("read_doubles_from_file error: size of input file '%s' does not "
               "match the given n.\n",
               fileName);
        printf("For n = %d the file size is expected to be (n * "
               "sizeof(double)) = %lu but the actual file size is %lu.\n",
    ...
    if (noOfItemsRead != fileSize) {
        printf("read_doubles_from_file error: failed to read file contents "
               "into buffer.\n");
        return -1;
    }
...
int main(int argc, const char *argv[]) {
    ...
    for (i = 0; i < N; i++) {
        ...
        if (buf1[i * 6 + 0] != buf2[i * 6 + 0]) {
            printf("Particle %d not equal\n", i);
            return -1;
        }
